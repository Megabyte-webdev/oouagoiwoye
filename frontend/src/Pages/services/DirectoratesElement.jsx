...
const DirectoratesElement = () => {
    const { id } = useParams()
    const check = DirectoratesData.find(item => item.title === id)
    const [data, setData] = useState();
    const navigate = useNavigate()
    function handleClick(event) {
        event.preventDefault();
        navigate('/services/directorate')
    }
    useEffect(() => {
        if (check) {
            setData(check)
        ...
        <div className='w-full h-auto lg:h-auto lg:min-h-screen px-2'>
            <div className='p-2 px-4 bg-white'>
                ...
                <div className='w-full flex items-center justify-center relative'>
                    ...
                    <h2 className='text-blue-700 text-2xl lg:text-4xl font-semibold z-0 absolute'>{data?.title}</h2>
                </div>
            ...
            <div className='mx-4 grid grid-cols-1 lg:grid-cols-2 gap-10'>
                <div className='w-full'>
                    <div className='w-full'>
                        <h2 className='text-black font-bold text-lg lg:text-2xl mt-2'>History of Academic Planning</h2>
