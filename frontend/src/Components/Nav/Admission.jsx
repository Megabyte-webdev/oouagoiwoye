    ...
    return (
        <div className='max-lg:w-full '>
            <li className={`group xl:px-2 px-3.5  flex flex-col items-center relative max-lg:items-start max-lg:px-0 max-lg:w-full`}>
                ...
                {admission && <div className={`cursor-pointer bg-white text-[#010035]  font-light z-50 py-8 text-md absolute px-8 left-[50%] translate-x-[-50%] top-[55px] group-hover:block max-lg:top-[0%] max-lg:left-[0%] max-lg:translate-x-[0%] max-lg:translate-y-[0%] max-lg:static max-lg:py-0 max-lg:w-[100%]`} >
                    ...
                    <ul className=" flex flex-col pt-7 gap-7 max-lg:px-0 max-lg:pt-3.5 font-500" >
                        {
                            navListJson.admissions.map(({ title }, index) => {
                                return <li className=' font-thin hover:opacity-[.77] w-[100%]' key={index}>
                                    <NavLink to='/admission' className='block w-full'></NavLink>{title}</li>
                            })
                        }
                    </ul>
