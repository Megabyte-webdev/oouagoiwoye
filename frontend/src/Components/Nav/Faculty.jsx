...
import { useFaculties  } from "../../hooks/useCampus";

const Faculty = () => {
    const { data: faculties } = useFaculties();
    console.log(faculties);
    const { setMenu, setAbout, setAdministration, faculty, setAdmission, setServices, setFaculty, setLogin } = useContext(NavContext);
        ...
        <div className='max-lg:w-full'>
            <li className={`group flex flex-col items-center relative max-lg:items-start max-lg:px-0 max-lg:w-full`}>
                <div className='flex gap-1 items-center max-lg:justify-between max-lg:px-8 max-lg:w-full max-md:px-7 max-lg:border-b-2 max-lg:py-7 z-[22]' onClick={toggleBtn}>
                    ...
                    <div>
                        {faculty ? <FaCaretUp /> : <FaCaretDown />}
                    </div>
                </div>

                {
                faculty && 
                <div className={`bg-white text-[#010035]  font-light z-50 py-8 text-md absolute px-8 left-[50%] translate-x-[-50%] top-[55px] group-hover:block max-lg:top-[0%] max-lg:left-[0%] max-lg:translate-x-[0%] max-lg:translate-y-[0%] max-lg:static max-lg:py-0 min-w-80`} >
