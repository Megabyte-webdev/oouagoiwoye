...
export default function Campus() {
    ...
    const breadcrumbs = [
        ...
        <Link key={2} className="text-blue-500" href="/services/campus">
            Campus
        </Link>,
    ];
    ...
    if (isError)
        return (
            <div className="p-4 text-red-600">Failed to load campus data.</div>
        );

    return (
        <>
            <Helmet>
                <title>OOU | Campuses</title>
            </Helmet>
            <div className="w-full h-auto lg:min-h-screen px-2">
                ...
                <div className="p-2 px-4">
                    {campusData.map((item, index) => (
                        <div
                            key={index}
                            className="w-full bg-white even:bg-blue-50 border-blue-900 border-t-2 last:border-b-2 p-2 flex items-center justify-between"
                        >
                            <ImageBadgeCard
                                image={
                                    item?.image
                            ...
                            <div className="w-10/12 h-full p-3 pl-5 lg:pl-10">
                                ...
                                <CustomButton
                                    handleClick={() =>
                                        navigate(`/services/campus/${item.id}`)
                                    }
                                />
                            </div>
                        </div>
